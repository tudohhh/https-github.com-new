import math, json, hashlib, time, os, urllib.request, urllib.parse, sqlite3
...
MARKET_THRESH_NATURAL=0.7031; MARKET_THRESH_ORCHESTRATED=0.6743; MARKET_P50=0.6862
...
def db_init(path):
    ...
    con.cursor().executescript("""
        ...
        CREATE TABLE IF NOT EXISTS snapshots(id INTEGER PRIMARY KEY AUTOINCREMENT,ts TEXT,pair TEXT,regime TEXT,market_score REAL,shannon REAL,pe REAL,autocorr REAL,hurst REAL,macro_regime TEXT,final_regime TEXT,integrity INTEGER,tri_status TEXT,market_flags TEXT,anomaly_log TEXT,blocked INTEGER,input_hash TEXT);
        CREATE TABLE IF NOT EXISTS alerts(id INTEGER PRIMARY KEY AUTOINCREMENT,ts TEXT,pair TEXT,regime TEXT,reason TEXT);
        CREATE TABLE IF NOT EXISTS paper_trades(id INTEGER PRIMARY KEY AUTOINCREMENT,ts_open TEXT,ts_close TEXT,pair TEXT,direction TEXT,price_open REAL,price_close REAL,pct_move REAL,cost_pct REAL,edge REAL,platform TEXT,signal_regime TEXT,macro_regime TEXT,status TEXT);
    """)
...
def db_open_trade(con,pair,direction,price,regime,macro_regime):
    con.cursor().execute("INSERT INTO paper_trades(ts_open,pair,direction,price_open,cost_pct,signal_regime,macro_regime,platform,status) VALUES(?,?,?,?,?,?,?,\"paper\",\"OPEN\")",(datetime.now(timezone.utc).isoformat(),pair,direction,price,0.0028,regime,macro_regime)); con.commit()
...
def market_composite(series,ws=60):
    sh=shannon_score(series,ws); pe=pe_score(series,ws); ac=autocorr_score(series,ws); h=hurst_exp(series)
...
def dll_v15(primary,macro_regime="NEUTRAL"):
    score,sh,pe,ac,h=market_composite(primary); adj=macro_adj(score,macro_regime); regime=get_regime(adj)
    ...
    return {"timestamp":datetime.now(timezone.utc).isoformat(),
            ...
            "hurst_label":h_label,"macro_regime":macro_regime,"integrity_flag":True,"market_flags":flags or [],
            "anomaly_log":f"sh={sh} pe={pe} ac={ac} H={h}[{h_label}] mkt={score} adj={adj} macro={macro_regime}"}
...
class PairGovernor:
    def __init__(self,pair,thr=3):
        self.pair=pair; self.thr=thr; self.history=deque(maxlen=50)
        self.blocked=False; self.reason=""; self.consec=0; self.open_trade_id=None
...
def load_frankfurter_live(base,target):
    ...
    return json.loads(fetch(url)).get("rates",{}).get(target)
...
def live_loop(con,interval_min=60,fetch_every_sec=3600,window=120,max_cycles=None):
    ...
    cycle=0; mcycle=0
    log("START v15 — 4 surse crypto, 3 surse forex FRED+ER+BCE")
    tg("DLL v15 pornit\nCrypto:Kraken+CG+Bybit+CoinCap\nForex:FRED+ExchangeRate+Frankfurter(BCE)")
    while True:
        cycle+=1
        ...
        if mcycle==1 or mcycle%24==0:
            ...
            for p,m in FOREX.items():
                try:
                    ...
                    db_insert(con,p,"frankfurter",fkf)
                    log(f"BCE {p}:{len(fkf)} zile"); time.sleep(0.3)
            ...
            try:
                ...
                if dec["regime"]=="ORCHESTRATED" and edge>0 and not gov.blocked:
                    if not gov.open_trade_id:
                        ...
                        gov.open_trade_id=tid; log(f"SHORT {pair} @ {prices.get('kraken',0):.4f} edge={edge}%")
                elif dec["regime"]=="NATURAL" and gov.open_trade_id:
                    e=db_close_trade(con,gov.open_trade_id,prices.get("kraken",0))
                    r="WIN" if e and e>0 else "LOSS"; log(f"CLOSE {pair} edge={e}% {r}")
                    tg(f"{r} {pair}\nEdge:{e}%\nMacro:{mr}"); gov.open_trade_id=None
                if dec["regime"]=="ORCHESTRATED":
                    tg(f"ORC {pair}\nscore={snap["market_score"]} PE={snap["pe"]}\nsrc={len(prices)} div={md:.3f}%\nMacro:{mr} F&G:{fg}")
        ...
        for pair,m in FOREX.items():
            gov=governors[pair]; prices={}
            try:
                fred_s=db_load(con,pair,"fred",window)
                fkf_s=db_load(con,pair,"frankfurter",window)
